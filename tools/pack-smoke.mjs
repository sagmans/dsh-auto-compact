 ...
 * the tarball users install must carry both plus every module the entry point
 * imports. The packed manifest and archive are inspected rather than the
 * checkout, because a path that only exists before packing would still break
 * the install.
 */
...
const PATCH_ROW = "name: '@sagmans/dsh-auto-compact'"

/** The shipped engine row this bundle must replace, never mount twice. */
const REPLACED_ROW = 'id: compaction-basic'
...
const problems = []
const out = mkdtempSync(join(tmpdir(), 'dsh-auto-compact-pack-'))
try {
  ...
  if (!patch.includes(REPLACED_ROW)) problems.push('the bundle patch no longer replaces ' + REPLACED_ROW)
